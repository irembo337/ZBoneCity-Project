...
namespace BonelabAdvancedHealth.MarrowEditor
{
    public static class MedicalItemSetupTool
    {
        ...
        private static void ConfigureMarrow(GameObject root, Collider mainCollider, Collider gripCollider, string itemType)
        {
            ...
            Component grip = AddComponentIfAvailable(root, IsCylinderItem(itemType) ? "SLZ.Marrow.CylinderGrip" : "SLZ.Marrow.BoxGrip");
            if (grip == null)
                return;

            Transform gripPoint = root.transform.Find("AHS_GripPoint");
            SetMember(grip, "targetTransform", gripPoint);
