    ...
    public static class MedicalItemPrefabBuilder
    {
        ...
        private static void BuildPrefab(ItemDefinition item)
        {
            ...
            try
            {
                ...
                CreateChild(root.transform, "AHS_GripPoint", Vector3.zero, item.Shape == PrimitiveShape.CapsuleZ ? Quaternion.Euler(0f, 90f, 0f) : Quaternion.identity);
                CreateGripVolume(root.transform, item);
                Debug.LogWarning("[ZBC] Placeholder medical prefabs are exported with safe physics only. Configure native Marrow InteractableHost/Grip in the SDK inspector for final pickup crates.");
